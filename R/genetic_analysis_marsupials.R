...
marsupials <- unique (ind_trait$species)[c(1,4,9,10,16)]

## pegar somente os atributos das sp de sigmodontine
ind_trait <- ind_trait[which (ind_trait$species %in% marsupials),]

# subset
gen_data  <- gen_data [which (ind_sequenciados %in% as.character(ind_trait$individuo) == T),]

# DNA INTO PHYDAT
gen_marsupials <- phyDat(gen_data, type = "DNA", levels = NULL)
gen_marsupials <- subset (gen_marsupials, attr (gen_data,"dimnames")[[1]] %in% ind_trait$individuo)
#class(gen_rodents)

## NJ tree
gen_marsupials_dnabin <- as.DNAbin (gen_marsupials)
# genetic distance
dist_gen <- dist.dna(gen_marsupials_dnabin, model="K80") # distance derived by Kimura (1980),
marsupials_NJ  <- nj(dist_gen)

## bootstrapping the tree
myBoots <- boot.phylo(marsupials_NJ, gen_marsupials_dnabin, function(e) 
  root(nj(dist.dna(e, model = "K80")),1))

plot(marsupials_NJ, show.tip=T, edge.width=2)
...
mt_marsupials <- modelTest(gen_marsupials)
save(mt_marsupials ,file = here ("output", "test_model_marsupials.RData"))

load(here ("output", "test_model_marsupials.RData"))
env <- attr(mt_marsupials, "env")
ls(envir=env)

round(mt_marsupials$AICw,4)
...
fitGTRopt.Mar <- optim.pml(fitGTR, model="GTR", optInv=TRUE, optGamma=TRUE,
                       rearrangement = "stochastic", control = pml.control(trace = 0))
save(here("output","fitGTRopt.Mar.RData"))
# bootstrap to have branch support
nsamples<- 10000
bs.GTR.Mar <- bootstrap.pml(fitGTRopt.Mar, bs= nsamples, optNni=TRUE,
                        control = pml.control(trace = 0),
                        optGamma=TRUE, optInv=TRUE, model="GTR")

save(here("output","bs.GTR.Mar.RData"))

## cons tree
#cons <- consensus(bs.GTR.Mar, p = 0.7, check.labels = TRUE)

# plot opt tree and branch support
pdf(here("output","phyGTRFanMar.pdf"))
par(mfrow=c(1,1))
plotBS (midpoint(fitGTRopt.Mar$tree),
        bs.GTR.Mar,
        p = 5, 
        type="fan",
        cex=0.5)
...
comparePhylo(marsupials_NJ, fitGTRopt.Mar$tree, 
             plot = TRUE, force.rooted = TRUE)

## depth of correction
dist_mat <- as.matrix(dist.p(gen_marsupials, 
                             cost = "polymorphism", 
...
mantel(dist_mat,
       as.dist(cophenetic(fitGTRopt.Mar$tree),diag=T))

## checking correspondence between distances
x <- as.vector(as.dist(cophenetic(marsupials_NJ)))
y <- as.vector(as.dist(cophenetic(fitGTRopt.Mar$tree)))
