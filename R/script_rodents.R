...
df_morfo_distances <- do.call(rbind, 
                              lapply (seq (1,ncol(all_ind_comb)), function (i)
                                ...
                                data.frame (ind1 = all_ind_comb[1,i],
                                            ind2 = all_ind_comb[2,i],
                                            DistMorpho = m_dist_morfo[which(rownames(m_dist_morfo) == all_ind_comb[1,i]),
...
with (df_morfo_distances, plot (as.factor(sex), DistMorpho,
                                xlab = "Sex",
...
amb_ocorr <- lapply (seq(1,ncol(occur)), function (especie)
  amb_data [occur[,especie] >= 1,c("samam","herb","arbus","arvor","serap","abdoss","drh","dafl")])
...
pdf(here("output", "continuous_genetics_rodents_ALL_COMPONENTS.pdf"),width = 5, height=5)

par(mfrow=c(1,1),mar=c(5, 4, 4, 6) + 0.1)
plot (intervals,res_df[,1],type="l",
      pch=19,ylim=c(-0.05,0.4),col="blue",
...
# ONLY INTERESTING COMPONENTS

pdf(here("output", "continuous_genetics_rodents_THREE_COMPONENTS.pdf"),width = 5, height=5)
...
write.csv (test_of_fraction_rodents,
           file= here("output", "test_of_fraction_rodents.csv"))
...
pdf(file=here("output","vectorized","rodent_geo_gen.pdf"),height=4,width=8)
